...
const STEP_LABELS: Record<string, string> = {
  ...
  "paddy doctor entry": "Checking doctor entrypoint",
  "paddy doctor": "Running doctor checks",
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this OpenClaw install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("paddy doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("paddy gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("paddy gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("paddy doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("paddy gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("paddy gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "openclaw.hearth.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          "Docs:",
        )} ${formatDocsLink("/cli/update", "openclaw.hearth.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
