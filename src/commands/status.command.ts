...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Paddy status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("paddy security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("paddy security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://openclaw.hearth.ai/faq");
  runtime.log("Troubleshooting: https://openclaw.hearth.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("paddy status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("paddy logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("paddy status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("paddy gateway probe")}`);
  }
}
