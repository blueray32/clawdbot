...
struct PaddyMacCLI {
    static func main() async {
        ...
        default:
            fputs("paddy-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      paddy-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      paddy-mac discover [--timeout <ms>] [--json] [--include-local]
      paddy-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      paddy-mac connect
      paddy-mac connect --url ws://127.0.0.1:18789 --json
      paddy-mac discover --timeout 3000 --json
      paddy-mac wizard --mode local
    """)
}
