...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: PaddyProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.paddy", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: PaddyKit.AnyCodable]()) {
                $0[$1.key] = PaddyKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: PaddyProtocol.AnyCodable?) -> [String: PaddyProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: PaddyProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: PaddyKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: PaddyProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("paddy.control.heartbeat")
    static let controlAgentEvent = Notification.Name("paddy.control.agent")
}
