...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Paddy token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: PaddyOAuthStore.AnthropicOAuthStatus = PaddyOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.paddy", category: "anthropic-oauth")
...
enum PaddyOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let paddyOAuthDirEnv = "OPENCLAW_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Paddy OAuth token file not found"
            case .unreadableFile: "Paddy OAuth token file not readable"
            case .invalidJSON: "Paddy OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Paddy OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Paddy OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.paddyOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".paddy", isDirectory: true)
