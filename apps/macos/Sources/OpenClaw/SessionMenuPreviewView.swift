...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.paddy", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> PaddySessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: PaddySessionPreviewEntry,
    ...
    private static func cache(payload: PaddySessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: PaddyChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [PaddyKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [PaddyKit.AnyCodable]) -> [PaddyChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(PaddyChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: PaddyChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: PaddyChatMessage) -> Bool {
    ...
    private static func toolNames(for message: PaddyChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: PaddyChatMessage) -> String? {
