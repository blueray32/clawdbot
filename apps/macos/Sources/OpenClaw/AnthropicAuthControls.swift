...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: PaddyOAuthStore.AnthropicOAuthStatus = PaddyOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: PaddyOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = PaddyOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = PaddyOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try PaddyOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Paddy can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: PaddyOAuthStore.AnthropicOAuthStatus,
