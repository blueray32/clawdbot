...
enum PaddyLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = PaddyOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = PaddyFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        PaddyLogging.bootstrapIfNeeded()
        let label = PaddyLogging.makeLabel(subsystem: subsystem, category: category)
...
struct PaddyOSLogHandler: LogHandler {
...
struct PaddyFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = PaddyLogging.parseLabel(self.label)
