...
enum DebugActions {
    private static let verboseDefaultsKey = "paddy.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "paddy.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = PaddyPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Paddy", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = PaddyPaths.configURL
        guard
