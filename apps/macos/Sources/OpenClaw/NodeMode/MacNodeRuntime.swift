...
actor MacNodeRuntime {
    ...
    func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        if self.isCanvasCommand(command), !Self.canvasEnabled() {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: PaddyNodeError(
                    code: .unavailable,
                    message: "CANVAS_DISABLED: enable Canvas in Settings"))
        }
        do {
            switch command {
            case PaddyCanvasCommand.present.rawValue,
                 PaddyCanvasCommand.hide.rawValue,
                 PaddyCanvasCommand.navigate.rawValue,
                 PaddyCanvasCommand.evalJS.rawValue,
                 PaddyCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case PaddyCanvasA2UICommand.reset.rawValue,
                 PaddyCanvasA2UICommand.push.rawValue,
                 PaddyCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleA2UIInvoke(req)
            case PaddyCameraCommand.snap.rawValue,
                 PaddyCameraCommand.clip.rawValue,
                 PaddyCameraCommand.list.rawValue:
                return try await self.handleCameraInvoke(req)
            case PaddyLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case MacNodeScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            case PaddySystemCommand.run.rawValue:
                return try await self.handleSystemRun(req)
            case PaddySystemCommand.which.rawValue:
                return try await self.handleSystemWhich(req)
            case PaddySystemCommand.notify.rawValue:
                return try await self.handleSystemNotify(req)
            case PaddySystemCommand.execApprovalsGet.rawValue:
                return try await self.handleSystemExecApprovalsGet(req)
            case PaddySystemCommand.execApprovalsSet.rawValue:
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case PaddyCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(PaddyCanvasPresentParams.self, from: req.paramsJSON)) ??
                PaddyCanvasPresentParams()
        ...
        case PaddyCanvasCommand.hide.rawValue:
            let sessionKey = self.mainSessionKey
            await MainActor.run {
                CanvasManager.shared.hide(sessionKey: sessionKey)
            }
            return BridgeInvokeResponse(id: req.id, ok: true)
        case PaddyCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(PaddyCanvasNavigateParams.self, from: req.paramsJSON)
            let sessionKey = self.mainSessionKey
            try await MainActor.run {
                _ = try CanvasManager.shared.show(sessionKey: sessionKey, path: params.url)
            }
            return BridgeInvokeResponse(id: req.id, ok: true)
        case PaddyCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(PaddyCanvasEvalParams.self, from: req.paramsJSON)
            let sessionKey = self.mainSessionKey
        ...
        case PaddyCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(PaddyCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case PaddyCanvasA2UICommand.push.rawValue,
             PaddyCanvasA2UICommand.pushJSONL.rawValue:
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        switch req.command {
        case PaddyCameraCommand.snap.rawValue:
            let params = (try? Self.decodeParams(PaddyCameraSnapParams.self, from: req.paramsJSON)) ??
                PaddyCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case PaddyCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(PaddyCameraClipParams.self, from: req.paramsJSON)) ??
                PaddyCameraClipParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case PaddyCameraCommand.list.rawValue:
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(PaddyLocationGetParams.self, from: req.paramsJSON)) ??
            PaddyLocationGetParams()
        ...
        do {
            ...
            let payload = PaddyLocationPayload(
    ...
    private func handleA2UIReset(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        (() => {
          const host = globalThis.paddyA2UI;
          if (!host) return JSON.stringify({ ok: false, error: "missing paddyA2UI" });
    ...
    private func handleA2UIPush(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        let messages: [PaddyKit.AnyCodable]
        if command == PaddyCanvasA2UICommand.pushJSONL.rawValue {
            let params = try Self.decodeParams(PaddyCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
            messages = try PaddyCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
        } else {
            do {
                let params = try Self.decodeParams(PaddyCanvasA2UIPushParams.self, from: req.paramsJSON)
        ...
        let messagesJSON = try PaddyCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
        let js = """
        (() => {
          try {
            const host = globalThis.paddyA2UI;
    ...
    private func resolveA2UIHostUrl() async -> String? {
        ...
        return baseUrl.appendingPathComponent("__paddy__/a2ui/").absoluteString + "?platform=macos"
    }
    ...
    private func handleSystemRun(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(PaddySystemRunParams.self, from: req.paramsJSON)
    ...
    private func handleSystemWhich(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(PaddySystemWhichParams.self, from: req.paramsJSON)
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(PaddySystemNotifyParams.self, from: req.paramsJSON)
...
extension MacNodeRuntime {
    ...
    private nonisolated static func locationMode() -> PaddyLocationMode {
        let raw = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        return PaddyLocationMode(rawValue: raw) ?? .off
    }
    ...
    private static func errorResponse(
        ...
        code: PaddyNodeErrorCode,
        message: String) -> BridgeInvokeResponse
    {
        BridgeInvokeResponse(
            id: req.id,
            ok: false,
            error: PaddyNodeError(code: code, message: message))
    }

    private static func encodeCanvasSnapshot(
        image: NSImage,
        format: PaddyCanvasSnapshotFormat,
        maxWidth: Int?,
