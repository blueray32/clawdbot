...
private let webChatSwiftLogger = Logger(subsystem: "ai.paddy", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: PaddyChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> PaddyChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(PaddyChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> PaddyChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                PaddyGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<PaddyChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any PaddyChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = PaddyChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: PaddyChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<PaddyChatView>) -> NSViewController
    {
