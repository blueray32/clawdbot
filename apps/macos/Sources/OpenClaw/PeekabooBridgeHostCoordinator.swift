...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.paddy", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: PaddyPeekabooBridgeServices?
    private static var paddySocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Paddy", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = PaddyPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.paddySocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.paddySocketPath, privacy: .public)")
    }
...
@MainActor
private final class PaddyPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.paddy.peekaboo")
