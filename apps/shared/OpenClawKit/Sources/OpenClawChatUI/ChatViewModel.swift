...
private let chatUILogger = Logger(subsystem: "ai.paddy", category: "PaddyChatUI")

@MainActor
@Observable
public final class PaddyChatViewModel {
    public private(set) var messages: [PaddyChatMessage] = []
    ...
    public var attachments: [PaddyPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [PaddyChatPendingToolCall] = []
    public private(set) var sessions: [PaddyChatSessionEntry] = []
    private let transport: any PaddyChatTransport
    ...
    private var pendingToolCallsById: [String: PaddyChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any PaddyChatTransport) {
    ...
    public var sessionChoices: [PaddyChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [PaddyChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [PaddyChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: PaddyPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [PaddyChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: PaddyChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [PaddyChatMessage]) -> [PaddyChatMessage] {
        var result: [PaddyChatMessage] = []
    ...
    private static func dedupeKey(for message: PaddyChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [PaddyChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> PaddyChatAttachmentPayload in
            PaddyChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> PaddyChatSessionEntry {
        PaddyChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: PaddyChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: PaddyChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: PaddyAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = PaddyChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> PaddyPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
