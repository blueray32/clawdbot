...
@MainActor
struct PaddyChatComposer: View {
    @Bindable var viewModel: PaddyChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(PaddyChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(PaddyChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \PaddyPendingAttachment.id)
                { (att: PaddyPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            PaddyPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(PaddyChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(PaddyChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(PaddyChatTheme.subtleCard)
        .clipShape(Capsule())
    }
