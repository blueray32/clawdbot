...
private final class TestChatTransport: @unchecked Sendable, PaddyChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [PaddyChatHistoryPayload]
    private let sessionsResponses: [PaddyChatSessionsListResponse]

    private let stream: AsyncStream<PaddyChatTransportEvent>
    private let continuation: AsyncStream<PaddyChatTransportEvent>.Continuation

    init(
        historyResponses: [PaddyChatHistoryPayload],
        sessionsResponses: [PaddyChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<PaddyChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> PaddyChatHistoryPayload {
        ...
        return self.historyResponses.last ?? PaddyChatHistoryPayload(
        ...
        attachments _: [PaddyChatAttachmentPayload]) async throws -> PaddyChatSendResponse
    {
        ...
        return PaddyChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> PaddyChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? PaddyChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: PaddyChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = PaddyChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = PaddyChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { PaddyChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = PaddyChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = PaddyChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = PaddyChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { PaddyChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
